...
} from "@/common/validation/plain.validation";
...
export const validateIBANField = (iban: string): FieldValidationResult => {
  if (!isStringValueInformed(iban)) {
...
export const validateAccountField = (accountId: string): FieldValidationResult => {
  if (!isStringValueInformed(accountId)) {
    return buildRequiredFieldValidationFailedResult();
  }
